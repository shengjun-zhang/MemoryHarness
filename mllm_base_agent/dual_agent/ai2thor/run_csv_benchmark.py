#!/usr/bin/env python3
"""
AI2-THOR dual-agent CSV benchmark runner.

Reuse SpatialWorld single-agent benchmark helpers, but execute `mllm_base_agent/dual_agent/ai2thor/main.py`.
"""
...
from scripts.ai2thor.run_benchmark import (
    count_csv_status,
...
def benchmark_run_dirname(prefix: str, timestamp: str, save_name: str | None) -> str:
    return f"{save_name}_{timestamp}" if save_name else f"{prefix}_{timestamp}"


def prepare_runtime_config(config_path: Path, headless: bool):
    return config_path, None, "headless" if headless else "normal"
...
def main():
    parser = argparse.ArgumentParser(description="AI2-THOR dual-agent CSV benchmark runner")
    parser.add_argument("--csv", type=str, required=True, help="CSV file with task IDs")
    parser.add_argument("--workers", type=int, default=4, help="Parallel workers")
    parser.add_argument("--config", type=str, default="experiments/configs/ai2thor/dual/config_close_gpt-5.yaml", help="Base config path")
