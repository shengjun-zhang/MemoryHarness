...
- --max-steps:      summary，      python -m scripts.ai2thor.work.run_task（    ，        ） 
...
def evaluate_task(task_id: str, config_path: str, headless: bool = False) -> bool:
    ...
    try:
        with EVAL_LOCK:
            cmd = [
                sys.executable,
                "scripts/evaluate_actions_ai2thor.py",
                "--task", task_id,
                "--config", config_path,
            ]
...
def normalize_task_id(task_id: str) -> str:
    ...
    if not task_id:
        return task_id
    tid = str(task_id).strip().lstrip("\ufeff")
    if '_' in tid and tid.startswith('ai2thor_'):
        return tid.replace('ai2thor_', 'ai2thor', 1)
    return tid
    ...
    try:
        ...
        for i in range(1, len(rows)):
            if not rows[i] or not rows[i][0].strip():
                continue
            csv_task_id = rows[i][0].strip().lstrip("\ufeff")
...
def decide_csv_status_from_result(result_info: Dict[str, Any], fallback_failure_type: str) -> Optional[str]:
    ...
    # HTTP / registry errors often appear only in fail_reason; older runs may have mis-set
    # failure_type=parse_error (e.g. OpenAI 404 "model does not exist"). These must stay null.
    if fail_reason:
        if re.search(
            r"(?:\berror\s+code\b|\bstatus\s*code\b)\s*:\s*(?:401|403|404|408|409|422|429|5\d{2})\b",
            fail_reason,
            re.I,
        ):
            return None
        fr_low = fail_reason.lower()
        if "does not exist" in fr_low and "model" in fr_low:
            return None
    ...
    if fail_reason:
        ...
        if "model determined" in fail_reason_lower or "refused to continue" in fail_reason_lower:
            return "false"
        #  ：              （  run      env_error）→    /   ，false
        #     ·Unity/    env_error，   fail_reason           
        if "called with invalid argument" in fail_reason_lower and "expected arguments" in fail_reason_lower:
            return "false"
        if "invalid action parameters:" in fail_reason_lower:
            return "false"
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog=r"""
Examples:
  python -m scripts.ai2thor.run_benchmark --csv "experiments/csv/ai2thor/Spatial-Annotation-ai2thor.csv" --workers 4 --config experiments/configs/ai2thor/config_close_gpt-5.yaml
  python -m scripts.ai2thor.run_benchmark --csv "experiments/csv/ai2thor/Spatial-Annotation-ai2thor.csv" --workers 4 --config experiments/configs/ai2thor/config_close_gpt-5.yaml --headless
  python -m scripts.ai2thor.run_benchmark --csv "experiments/csv/ai2thor/Spatial-Annotation-ai2thor.csv" --workers 4 --config experiments/configs/ai2thor/config_close_gpt-5.yaml --max-steps 50
  python -m scripts.ai2thor.run_benchmark --csv "experiments/csv/ai2thor/Spatial-Annotation-ai2thor.csv" --sequential --config experiments/configs/ai2thor/config_close_gpt-5.yaml --headless
  python -m scripts.ai2thor.run_benchmark --csv "experiments/csv/ai2thor/Spatial-Annotation-ai2thor.csv" --sequential --config experiments/configs/ai2thor/config_close_gpt-5.yaml --skip-completed
  python -m scripts.ai2thor.run_benchmark --csv "experiments/csv/ai2thor/Spatial-Annotation-ai2thor.csv" --workers 1 --config experiments/configs/ai2thor/config_close_gpt-5.yaml --task ai2thor03017
  # 重新运行所有任务（不做断点续跑/resume，强制重跑 CSV 中所有任务，包括已完成的）:
  python -m scripts.ai2thor.run_benchmark --csv "experiments/csv/ai2thor/Spatial-Annotation-ai2thor.csv" --config experiments/configs/ai2thor/config_close_gpt-5.yaml --rerun-all
  #                 （CSV   config       ）:
  python -m scripts.ai2thor.run_benchmark --csv "experiments/csv/ai2thor/Spatial-Annotation-ai2thor-Gemini-2.5-pro.csv" --config experiments/configs/ai2thor/config_close_Gemini-2.5-pro.yaml --save-name Gemini-2.5-pro --workers 4
  python -m scripts.ai2thor.run_benchmark --csv "experiments/csv/ai2thor/Spatial-Annotation-ai2thor-Gemini-3-Pro-Preview.csv" --config "experiments/configs/ai2thor/config_close_Gemini-3-Pro-Preview.yaml" --save-name "Gemini 3-Pro-Preview" --workers 4
  python -m scripts.ai2thor.run_benchmark --csv "experiments/csv/ai2thor/Spatial-Annotation-ai2thor-Gemini-3-Flash-Preview.csv" --config "experiments/configs/ai2thor/config_close_Gemini-3-Flash-Preview.yaml" --save-name Gemini-3-Flash-Preview --workers 4
  python -m scripts.ai2thor.run_benchmark --csv "experiments/csv/ai2thor/Spatial-Annotation-ai2thor-gpt-5.csv" --config experiments/configs/ai2thor/config_close_gpt-5.yaml --save-name gpt-5 --workers 4
  python -m scripts.ai2thor.run_benchmark --csv "experiments/csv/ai2thor/Spatial-Annotation-ai2thor-gpt-5.csv" --config experiments/configs/ai2thor/config_close_gpt-5.yaml --save-name Gpt-5p4 --workers 4
  python -m scripts.ai2thor.run_benchmark --csv "experiments/csv/ai2thor/Spatial-Annotation-ai2thor-Claude-sonnet-4p6.csv" --config experiments/configs/ai2thor/config_close_Claude-sonnet-4p6.yaml --save-name Claude-sonnet-4p6 --workers 4
  python -m scripts.ai2thor.run_benchmark --csv "experiments/csv/ai2thor/Spatial-Annotation-ai2thor-gpt-5.csv" --config experiments/configs/ai2thor/config_close_gpt-5.yaml --save-name doubao_2 --workers 4
  python -m scripts.ai2thor.run_benchmark --csv "experiments/csv/ai2thor/Spatial-Annotation-ai2thor-Kimi-K2.5.csv" --config experiments/configs/ai2thor/config_close_kimi-k25.yaml --save-name kimi_k25 --workers 4
  python -m scripts.ai2thor.run_benchmark --csv "experiments/csv/ai2thor/Spatial-Annotation-ai2thor-qwen3.5.csv" --config experiments/configs/ai2thor/config_close_qwen-3p5.yaml --save-name qwen_3p5 --workers 4
        """
    )
    ...
    parser.add_argument(
        ...
        help="Config file path (default: experiments/configs/ai2thor/config_close_gpt-5.yaml)"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Optional; for benchmark runs this is NOT passed to python -m scripts.ai2thor.work.run_task (each task uses 10+2*golden_actions.steps)"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Benchmark         ，  Gemini-2.5-pro；      benchmark   benchmark_sequential"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Run only this task ID (e.g. ai2thor03017). Otherwise run all null tasks in CSV"
    )
    ...
    if args.task:
        ...
        if not task_ids:
            if only_null:
                print("✓ No task IDs with Completed=null — this CSV has nothing pending (exit 0, success).")
                print("💡 All rows may already be true/false; reset Completed or use --task to re-run specific IDs.")
                sys.exit(0)
            else:
    ...
    def execute_task(task_id: str) -> dict:
        ...
        try:
            ...
            if check_task_success(task_id, task_output_dir, str(actual_config_path), args.headless):
                ...
                task_status = "success"
                save_task_log(task_id, task_log, task_logs_dir, 1, "success")
                copied_ok = copy_to_outputs_completed(
                    task_id, task_output_dir, str(outputs_completed_path)
                )
                if task_result_info.get('task_result') == 'failure':
                    csv_status = 'false'
                else:
                ...
                if copied_ok:
                    print(
                        f"  ✅ {task_id} success (evaluate passed) -> copied and CSV updated({csv_status}, +extra)"
                    )
                else:
                    print(
                        f"  ✅ {task_id} success (evaluate passed) -> CSV updated({csv_status}, +extra); "
                        f"⚠️ copy to outputs_completed failed (see log above)"
                    )
            else:
                failure_reason = determine_failure_reason(result_json_path=result_json)
                csv_status = decide_csv_status_from_result(task_result_info, failure_reason)
                ...
                if result_json:
                    if failure_reason == "api_error":
                        ...
                        if csv_status == 'false' and copy_to_outputs_completed(task_id, task_output_dir, str(outputs_completed_path)):
                            ...
                            print(f"  ❌ {task_id} Parse error -> {csv_status}")
                        else:
                            update_csv_task_record(
                                csv_path,
                                task_id,
                        ...
                        if csv_status == 'false' and copy_to_outputs_completed(task_id, task_output_dir, str(outputs_completed_path)):
                            ...
                            print(f"  ❌ {task_id} Invalid action -> {csv_status}")
                        else:
                            update_csv_task_record(
                                csv_path,
                                task_id,
                        ...
                        if csv_status == 'false' and copy_to_outputs_completed(task_id, task_output_dir, str(outputs_completed_path)):
                            ...
                            print(f"  ❌ {task_id} Evaluate failed -> {csv_status}")
                        else:
                            update_csv_task_record(
                                csv_path,
                                task_id,
                    ...
                    if csv_status and task_status == "failed_external":
                        # task_result=failure should still be aligned to false even when failure_type=api/env
                        if csv_status == 'false' and copy_to_outputs_completed(task_id, task_output_dir, str(outputs_completed_path)):
                            update_csv_task_record(
                                csv_path,
                                task_id,
