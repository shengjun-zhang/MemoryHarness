#!/usr/bin/env bash
# HOPE/AFO multi-worker launcher for SpatialWorld single-agent benchmarks.
#
# Mirrors hope/launch_dual_benchmark.sh but drives the single-agent CSV
# benchmark runners (scripts.ai2thor.run_benchmark /
# scripts.procthor.run_benchmark) instead of the dual-agent ones. Sharding,
# environment variables (Vulkan/lavapipe, AI2-THOR build cache, proxy), and
# the shared-timestamp handshake across workers all follow the same scheme
# as the dual-agent launcher so both can be operated the same way.
...
while [ "$#" -gt 0 ]; do
    case "$1" in
        ...
        --help|-h)
            cat <<'EOF'
Usage: launch_single_benchmark.sh [ai2thor|procthor] [options]
...
Options:
  ...
  -d, --decision-mode MODE         Per-step decision strategy: standard (default) or lookahead
  -w, --history-window-size STEPS  Short-term multimodal history window size (steps)
  -h, --help                       Show this help

The same values can be supplied with IMAGE_SCALE, IMAGE_RECENT_STEPS,
DECISION_MODE and HISTORY_WINDOW_SIZE. Most other behavior is controlled via
environment variables (see script body), e.g. CSV_FILE, CONFIG_FILE, SAVE_NAME,
TASK_TIMEOUT_SECONDS, SEQUENTIAL, RERUN_ALL, MAX_STEPS.

Set RERUN_ALL=1 to force re-running every task in the CSV (including rows
already marked true/false), instead of the default resume behavior that
skips already-completed tasks.

Use -w/--history-window-size to shrink the number of history steps (and thus
history images) sent to the VLM each step, which helps avoid HTTP 413 Request
Entity Too Large on long episodes (especially combined with -d lookahead).
EOF
            exit 0
            ;;
        *)
...
if [ -n "$DECISION_MODE" ] && [ "$DECISION_MODE" != "standard" ] && [ "$DECISION_MODE" != "lookahead" ]; then
    echo "Invalid decision mode: $DECISION_MODE (expected standard or lookahead)" >&2
    exit 2
fi
...
case "$BENCHMARK_ENV" in
    ai2thor)
        ...
        MODULE="scripts.ai2thor.run_benchmark"
        CSV_FILE="${CSV_FILE:-$REPO_DIR/experiments/csv/ai2thor/Spatial-Annotation-ai2thor-doubao-2.0-lite.csv}"
        CONFIG_FILE="${CONFIG_FILE:-$REPO_DIR/experiments/configs/ai2thor/config_close_doubao-2.yaml}"
        OUTPUT_DIR="${OUTPUT_DIR:-$REPO_DIR/scripts/ai2thor/benchmark_outputs}"
        OUTPUTS_COMPLETED_DIR="${OUTPUTS_COMPLETED_DIR:-$REPO_DIR/scripts/ai2thor/outputs_completed}"
        ;;
    procthor)
        ENV_DIR="$PROCTHOR_ENV_DIR"
        MODULE="scripts.procthor.run_benchmark"
        CSV_FILE="${CSV_FILE:-$REPO_DIR/experiments/csv/procthor/Spatial-Annotation-procthor-Gpt-5p4.csv}"
        CONFIG_FILE="${CONFIG_FILE:-$REPO_DIR/experiments/configs/procthor/config_close_doubao-2.yaml}"
        OUTPUT_DIR="${OUTPUT_DIR:-$REPO_DIR/scripts/procthor/benchmark_outputs}"
        OUTPUTS_COMPLETED_DIR="${OUTPUTS_COMPLETED_DIR:-$REPO_DIR/scripts/procthor/outputs_completed}"
        ;;
    *)
...
if [ -n "$HISTORY_WINDOW_SIZE" ] && ! [[ "$HISTORY_WINDOW_SIZE" =~ ^[0-9]+$ ]]; then
    log "Invalid history window size: $HISTORY_WINDOW_SIZE (expected a non-negative integer)"
    exit 2
fi
if [ ! -f "$CSV_FILE" ]; then
...
SAVE_NAME="${SAVE_NAME:-${BENCHMARK_ENV}_single_cluster}"

# All workers share one experiment timestamp, so their shard directories are
# grouped under OUTPUT_DIR/<save_name>_<time>/shard-XXX-of-YYY/ (mirrors
# launch_dual_benchmark.sh, keeping single-agent and dual-agent output layouts
# consistent instead of scattering per-shard timestamps directly under
# OUTPUT_DIR).
...
log "===== SpatialWorld distributed single-agent benchmark ====="
...
log "task_timeout_seconds=$TASK_TIMEOUT_SECONDS"
...
if [ -n "$DECISION_MODE" ]; then
    log "decision_mode=$DECISION_MODE"
fi
if [ -n "$HISTORY_WINDOW_SIZE" ]; then
    log "history_window_size=$HISTORY_WINDOW_SIZE"
fi
if [ "${RERUN_ALL:-0}" = "1" ]; then
    log "rerun_all=1 (re-running all tasks, resume/skip-completed disabled)"
fi
...
if [ -n "$DECISION_MODE" ]; then
    cmd+=(--decision-mode "$DECISION_MODE")
fi
if [ -n "$HISTORY_WINDOW_SIZE" ]; then
    cmd+=(--history-window-size "$HISTORY_WINDOW_SIZE")
fi
if [ -n "${MAX_STEPS:-}" ]; then
